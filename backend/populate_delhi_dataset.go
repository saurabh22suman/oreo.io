...
func main() {
	...
	// Specific dataset to populate - Delhi Air Pollution
	datasetID := "da34f164-cff1-4b78-801c-7f19931255ed"
	filePath := "../uploads/da34f164-cff1-4b78-801c-7f19931255ed_final_dataset.csv"
	...
	var count int
	err = db.Get(&count, "SELECT COUNT(*) FROM dataset_data WHERE dataset_id = $1", datasetID)
	if err != nil {
		log.Fatalf("Failed to check existing data: %v", err)
	}
	
	if count > 0 {
		log.Printf("Dataset already has %d rows. Skipping population.", count)
		return
	}
